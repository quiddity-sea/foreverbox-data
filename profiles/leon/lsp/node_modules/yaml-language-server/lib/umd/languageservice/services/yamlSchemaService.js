...
(function (factory) {
    ...
    class YAMLSchemaService {
        ...
        async loadJSONSchema(url) {
            ...
            try {
                ...
                const errorCode = jsonLanguageTypes_1.ErrorCode.SchemaResolveError + (typeof code === 'number' && code < 0x10000 ? code : 0);
        ...
        async loadSchema(schemaUri) {
            ...
            if (unresolvedJsonSchema.errors &&
                ...
                try {
                    ...
                    if (!content) {
                        const errorMessage = l10n.t("Unable to load schema from '{0}': No content. {1}", toDisplayString(schemaUri), unresolvedJsonSchema.errors.map((error) => error.message).join(', '));
                        return new UnresolvedSchema({}, [toDiagnostic(errorMessage, jsonLanguageTypes_1.ErrorCode.SchemaResolveError, schemaUri)]);
                    }
                    try {
            ...
            if (this.schemaUriToNameAndDescription.has(schemaUri)) {
                ...
                return new UnresolvedSchema({}, [toDiagnostic(errorMessage, schemaError.code, schemaUri)]);
            }
        ...
        registerExternalSchema(uri, filePatterns, unresolvedSchema, name, description, versions) {
            ...
            const config = { uri: uri, fileMatch: filePatterns, schema: unresolvedSchema };
    ...
    function normalizeSchemaDraftUri(uri) {
        if (!uri)
            return '';
        let s;
    ...
    function knownSchemaDraftFromUri(schemaUri) {
        if (schemaUri === normalizeSchemaDraftUri(ajv4.defaultMeta()))
            return jsonLanguageTypes_1.SchemaDraft.v4;
        if (schemaUri === normalizeSchemaDraftUri(ajv7.defaultMeta()))
            return jsonLanguageTypes_1.SchemaDraft.v7;
        if (schemaUri === normalizeSchemaDraftUri(ajv2019.defaultMeta()))
            return jsonLanguageTypes_1.SchemaDraft.v2019_09;
        if (schemaUri === normalizeSchemaDraftUri(ajv2020.defaultMeta()))
            return jsonLanguageTypes_1.SchemaDraft.v2020_12;
        return undefined;
    }
    ...
    async function pickSchemaDraft($schema, loadSchema) {
        if (!$schema)
            return undefined;
    ...
    function pickMetaValidator(schemaDraft) {
        switch (schemaDraft) {
            case jsonLanguageTypes_1.SchemaDraft.v4:
                return schema04Validator;
            case jsonLanguageTypes_1.SchemaDraft.v7:
                return schema07Validator;
            case jsonLanguageTypes_1.SchemaDraft.v2019_09:
                return schema2019Validator;
            case jsonLanguageTypes_1.SchemaDraft.v2020_12:
