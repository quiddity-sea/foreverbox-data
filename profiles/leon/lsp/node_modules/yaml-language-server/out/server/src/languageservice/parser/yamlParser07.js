...
exports.defaultOptions = exports.SingleYAMLDocument = exports.YAMLDocument = void 0;
exports.parse = parse;
...
function parse(text, parserOptions = exports.defaultOptions, document) {
    ...
    const yamlDocs = Array.from(docs, (doc, index) => parsedDocToSingleYAMLDocument(doc, lineCounter, documentHeaderComments[index] ?? []));
    // Consolidate the SingleYAMLDocs
    return new yaml_documents_1.YAMLDocument(yamlDocs, tokensArr);
}
function parsedDocToSingleYAMLDocument(parsedDoc, lineCounter, documentHeaderComments) {
...
function getDocumentHeaderComments(tokens) {
    const documentHeaderComments = [];
    let pendingComments = [];
