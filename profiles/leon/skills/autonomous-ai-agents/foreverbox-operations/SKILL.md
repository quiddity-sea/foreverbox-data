...
- **Skill sharing**: Council-library skills are NOT exclusive to any agent. There are no "Leon-only" skills. All three HANDBOOK §8 skills (`council-library-cli`, `quiddity-folder-manager`, `foreverbox-operations`) must be present in every Lead agent profile. When provisioning a new agent or auditing existing ones, verify all three are present. The HANDBOOK §8 list is the canonical reference for which skills must be shared.
...
- `docs/HANDBOOK.md` — Operational Handbook: complete installation, API reference, model routing table, all tools, file organisation, wolf dispatch, troubleshooting. **Always consult this first before assuming something is missing — the handbook documents what exists.**
...
| 10 | `skills/foreverbox/quiddity-folder-manager/` | ✗ | `cp -r profiles/leon/skills/foreverbox/quiddity-folder-manager profiles/<agent>/skills/foreverbox/` |
| 11 | `skills/autonomous-ai-agents/foreverbox-operations/` | ✗ | `cp -r profiles/leon/skills/autonomous-ai-agents/foreverbox-operations profiles/<agent>/skills/autonomous-ai-agents/` |

The HANDBOOK §8 lists three skills that must be copied from Leon to all agents. Steps 9-11 cover all three. Steps 7 and 8 remain the most commonly missed — both are required for the memory provider to activate.
