...
metadata:
  hermes:
    ...
    related_skills: [hermes-agent, council-library-cli, quiddity-folder-manager]
---
...
- 3 specialist Wolves per Lead Agent.
- Stateless background workers; no personality, no human chat.
- Claim tasks from `agent_registry.task_queue` via **atomic claim pattern** (row-level locking alone is insufficient — must `UPDATE ... SET status='claimed' WHERE status='queued' AND id=...`).
- Write to Lead's Sanctum (`wolf_working_memory`, `memory_lore` with `source_type='wolf_synthesis'`).
...
| **Wolves** (all) | `google/gemini-3.1-flash-lite` | `deepseek/deepseek-v4-flash` | `deepseek/deepseek-v4-flash` (capped at Layer 2) |
...
The `council-library` project is a private monorepo on GitHub under the `quiddity-sea` account. Structure:
...
- `references/wolf-llm-routing.md` — Wolf workers with CognitiveRouter model tiers: task load estimation, tier selection, LLM execution via OpenRouter, memory_lore writes, systemd configuration with API keys.
- `docs/HANDBOOK.md` — Operational Handbook: complete installation, API reference, model routing table, all tools, file organisation, wolf dispatch, troubleshooting.
...
### 11. Agent Provisioning — Council-Library Readiness Checklist
...
**Checklist (10 items, all required):**
...
| 9 | `skills/foreverbox/council-library-cli/` | ✗ | `cp -r profiles/leon/skills/foreverbox/council-library-cli profiles/<agent>/skills/foreverbox/` |
| 10 | `SOUL.md` — first-person embodiment directive | ✗ | Add after identity line. See Procedure 10. |

**Pitfall**: Steps 7 and 8 are the most commonly missed. The foreverbox plugin provides 10 tools (memory_search, wolf_dispatch, commons_search, etc.) and a system_prompt_block that tells the agent about the Council Library. But the plugin is inert until BOTH conditions are met: (a) `hermes plugins enable foreverbox` AND (b) `memory.provider: foreverbox` in config.yaml. Without step 7, the plugin shows as "not enabled" in `hermes plugins list`. Without step 8, the memory provider hook never fires and the tools never load. Both are required — neither alone is sufficient.
